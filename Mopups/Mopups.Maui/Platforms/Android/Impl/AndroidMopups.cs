...
public class AndroidMopups : IPopupPlatform
{
    ...
    void HandleAccessibility(bool showPopup, bool disableAccessibilityHandling, PopupPage popup)
    {
        ...
        if (showPopup)
        {
            ...
            if (mainPageAndroidView is not null && mainPageAndroidView.ImportantForAccessibility !=
            ...
            {
                var androidView =
                    mainPage.Navigation.NavigationStack[navCount - 1]?.Handler?.PlatformView as Android.Views.View;

                if (androidView is not null &&
                    androidView.ImportantForAccessibility != ImportantForAccessibility.NoHideDescendants)
                {
            ...
            {
                var androidView =
                    mainPage.Navigation.ModalStack[modalCount - 1]?.Handler?.PlatformView as Android.Views.View;
                if (androidView is not null &&
                    androidView.ImportantForAccessibility != ImportantForAccessibility.NoHideDescendants)
                {
                    views.Add(androidView);
            ...
            {
                var androidView =
                    MopupService.Instance.PopupStack[popupCount - 2]?.Handler?.PlatformView as Android.Views.View;
                if (androidView is not null &&
                    androidView.ImportantForAccessibility != ImportantForAccessibility.NoHideDescendants)
                {
                    views.Add(androidView);
        ...
        static void ProcessView(bool showPopup, Android.Views.View? view)
        {
            ...
            view.ImportantForAccessibility =
                showPopup ? ImportantForAccessibility.NoHideDescendants : ImportantForAccessibility.Auto;
